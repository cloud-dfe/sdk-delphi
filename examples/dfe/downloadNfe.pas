...
procedure TForm1.ButtonDownloadNfeClick(Sender: TObject);
var
  Resp: string;
  JsonResp, DocObj: TJSONObject;
  XMLDecoded, PDFDecoded: string;
  Payload: TJSONObject;
begin
  ...
  try
    ...
    Resp := IntegraDfe.DownloadNfe(Payload);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          PDFDecoded := TIntegraUtil.Decode(TIntegraUtil.GetValueFromJson(DocObj, 'pdf'));

          ShowMessage('XML Decodificado: ' + XMLDecoded);
          ShowMessage('PDF Decodificado (Bytes): ' + IntToStr(Length(PDFDecoded)));
        end
        else
